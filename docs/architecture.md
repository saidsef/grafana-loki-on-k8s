...
Alloy is the single collector every backend sits behind. It runs as a DaemonSet, so one instance per node scrapes the local kubelet, tails the local pod logs, accepts OTLP from applications on that node, and scrapes pprof endpoints off pods carrying the profiling annotations. Metrics fan out to Prometheus and Mimir together rather than to one of them, since Prometheus holds the short retention that alert rules evaluate against and Mimir holds the long one. See [Alloy](alloy.md).
...
Each backend owns one signal and exposes one query language, and Grafana holds a provisioned data source per backend. Mimir is the default data source, so a dashboard that names no data source reads long-term metrics.
